db.define_table('User_Status',
    Field('user_id', db.auth_user, ondelete = 'NO ACTION', writable = False, readable = False),
    Field('username', unique = True, readable = False),
    Field('password', 'string', length=512, readable=False, label='Password'), # required
...
db.define_table('Master_Account', # standy by for accounts/finance module account code each location
    Field('account_sub_group_id','reference Master_Chart_Account_Prefix_Serial', ondelete = 'NO ACTION', requires = IS_EMPTY_OR(IS_IN_DB(db, db.Master_Chart_Account_Prefix_Serial.id,'%(serial_key)s, %(account_group_name)s - %(prefix_name)s', zero = 'Choose Account Sub-Group'))),     
...
db.define_table('Stock_Adjustment',
    ...
    Field('stock_adjustment_code_id','reference Master_Account', ondelete = 'NO ACTION',label = 'Account Code', requires = IS_EMPTY_OR(IS_IN_DB(db, db.Master_Account.id, '%(account_name)s', zero = 'Choose Account'))),    # create normal location code field    
...
db.define_table('Customer_Credit_Limit',
    ...
    Field('dept_code_id','reference Department', ondelete = 'NO ACTION', label = 'Dept Code',requires = IS_EMPTY_OR(IS_IN_DB(db, db.Department.id,'%(dept_code)s - %(dept_name)s', zero = 'Choose Department', error_message='Field should not be empty'))),    
...
db.define_table('Sales_Order',       
    ...
    Field('sales_man_id', 'reference Sales_Man', ondelete = 'NO ACTION', requires = IS_IN_DB(db, db.Sales_Man.id, '%(sales_man_name)s', zero = 'Choose Salesman')),   
    Field('returned','boolean',default = False),
    Field('delivered','boolean',default = False),    
    Field('delivered_by','reference Master_Account', ondelete = 'NO ACTION',label = 'Delivered By', requires = IS_EMPTY_OR(IS_IN_DB(db(db.Master_Account.master_account_type_id == 'E'), db.Master_Account.id, '%(account_code)s - %(account_name)s', zero = 'Choose Customer'))),    # create normal
    Field('status_id','reference Stock_Status',ondelete = 'NO ACTION', requires = IS_IN_DB(db, db.Stock_Status.id, '%(description)s', zero = 'Choose Status')),   
...
db.define_table('Sales_Return',
    ...
    Field('sales_man_on_behalf', 'reference Sales_Man', ondelete = 'NO ACTION', requires = IS_EMPTY_OR(IS_IN_DB(db, db.Sales_Man.id, '%(sales_man_name)s', zero = 'Choose Salesman On Behalf'))),
    ...
    Field('updated_on', 'datetime', update=request.now, writable = False, readable = False),
    Field('updated_by', 'reference auth_user', ondelete = 'NO ACTION',update=auth.user_id, writable = False, readable = False))
...
db.define_table('Sales_Invoice',       
    ...
    Field('delivered','boolean',default = False),    
    Field('delivered_by','reference Master_Account', ondelete = 'NO ACTION',label = 'Delivered By', requires = IS_EMPTY_OR(IS_IN_DB(db(db.Master_Account.master_account_type_id == 'E'), db.Master_Account.id, '%(account_code)s - %(account_name)s', zero = 'Choose Customer'))),        
...
dc.define_table('Almeera_Sales_Invoice_Temporary_Transaction',
    ...
    Field('wholesale_price_pcs', 'decimal(20,6)', default = 0), # per pcs.without tax
    Field('retail_price_pcs', 'decimal(20,6)',default = 0), # per pcs.without tax
    Field('price_cost_after_discount','decimal(20,6)',default = 0), 
    Field('selective_tax','decimal(20,6)', default = 0, label = 'Selective Tax'), # outer    
    Field('selective_tax_foc','decimal(20,6)', default = 0, label = 'Selective Tax'), # outer
    Field('selective_tax_price','decimal(20,6)', default = 0, label = 'Selective Tax'), # from item_prices
    Field('packet_selective_tax','decimal(20,6)', default = 0, label = 'Selective Tax'), # packet
...
db.define_table('POS_End_User',
    ...
    Field('status_id','reference Record_Status',ondelete = 'NO ACTION', requires = IS_IN_DB(db, db.Record_Status.id, '%(status)s', zero = 'Choose Status')),
    Field('created_on', 'datetime', default=request.now, writable = False, readable = False),
    Field('created_by', 'reference auth_user', ondelete = 'NO ACTION',default = auth.user_id, writable = False),
    Field('updated_on', 'datetime', update=request.now, writable = False, readable = True),
    Field('updated_by', db.auth_user,ondelete = 'NO ACTION', update=auth.user_id, writable = False, readable = False))

db.define_table('Delivery_Men',
    Field('account_code', 'string', length = 10),
    Field('first_name','string',length = 50),
    Field('last_name','string',length = 50),
    Field('status_id','reference Record_Status',ondelete = 'NO ACTION', requires = IS_IN_DB(db, db.Record_Status.id, '%(status)s', zero = 'Choose Status')),
...
db.define_table('Stock_Request',       
    ...
    Field('cancelled_on', 'datetime', default=request.now, writable = False, readable = False),
    Field('section_id','string',length=25,requires = IS_IN_SET([('F','Food Section'),('N','Non-Food Section')],zero ='Choose Section')),
    Field('received_by','reference POS_End_User', ondelete = 'NO ACTION',requires = IS_IN_DB(db(db.POS_End_User.status_id == 1), db.POS_End_User.id, '%(first_name)s %(last_name)s, %(account_code)s', zero = 'Choose Account')),
...
db.define_table('Other_Payment_Schedule',
...
db.define_table('Direct_Purchase_Receipt',  
    ...
    Field('dept_code_id','reference Department', ondelete = 'NO ACTION',label = 'Dept Code',requires = IS_IN_DB(db, db.Department.id,'%(dept_code)s - %(dept_name)s', zero = 'Choose Department')),
    Field('location_code_id','reference Location', ondelete = 'NO ACTION', label = 'Stock Source', requires = IS_IN_DB(db(db.Location.status_id == 1), db.Location.id, '%(location_code)s - %(location_name)s', zero = 'Choose Location')),
    Field('supplier_reference_order','string', length = 25),    
    Field('supplier_code_id', 'reference Supplier_Master',ondelete = 'NO ACTION', label = 'Supplier Code', requires = IS_IN_DB(db, db.Supplier_Master.id,'%(supp_code)s - %(supp_name)s', zero = 'Choose Supplier Code')),    
...
db.define_table('Payment_Category_Group',
...
db.define_table('Document_Register',     
    ...
    Field('trade_terms_id', 'reference Supplier_Trade_Terms', ondelete = 'NO ACTION',label = 'Trade Terms', requires = IS_IN_DB(db, db.Supplier_Trade_Terms.id, '%(trade_terms)s', zero = 'Choose Terms')),  #'string', length = 25, requires = IS_IN_SET(['EX-WORKS','FOB','C&F','CIF','LANDED COST'], zero = 'Choose Terms')),    
    Field('currency_id', 'reference Currency', ondelete = 'NO ACTION', requires = IS_IN_DB(db, db.Currency.id,'%(mnemonic)s - %(description)s', zero = 'Choose Currency')),
    Field('freight_currency_id', 'reference Currency', ondelete = 'NO ACTION', requires = IS_EMPTY_OR(IS_IN_DB(db, db.Currency.id,'%(mnemonic)s - %(description)s', zero = 'Choose Currency'))),
    Field('freight_amount', 'decimal(20,6)', default = 0),
    ...
    Field('payment_category_id','reference Payment_Category_Group', ondelete = 'NO ACTION', requires = IS_EMPTY_OR(IS_IN_DB(db, db.Payment_Category_Group,'%(payment_category)s', zero = 'Choose Payment Category'))),  
    Field('supplier_bank_id','reference Supplier_Bank',ondelete = 'NO ACTION', requires = IS_EMPTY_OR(IS_IN_DB(db, db.Supplier_Bank.id, '%(bank_name)s', zero = 'Choose Bank Name'))), # remove 
...
db.define_table('Purchase_Warehouse_Receipt',            
    Field('purchase_request_no_prefix_id', 'reference Transaction_Prefix', ondelete = 'NO ACTION',writable = False),   
    Field('purchase_request_no', 'integer', writable = False),
    Field('purchase_request_approved_by','reference auth_user', ondelete = 'NO ACTION',writable = False),
    Field('purchase_request_date_approved','date', writable = False),
...
db.define_table('Purchase_Warehouse_Receipt_Transaction',    
...
#---------- P  R  I  C  E   C  I  R  C  U  L  A  R  S c h e m a -------
db.define_table('Price_Circular',
    Field('transaction_no', 'integer', default = 0, writable = False),
    Field('transaction_date', 'date', default=request.now),
    Field('circular_no_prefix_id', 'reference Transaction_Prefix', ondelete = 'NO ACTION',writable = False),
    Field('circular_no','integer'),
    Field('circular_date', 'date', default = request.now),  
    Field('circulary_type','string',length=25,requires = IS_IN_SET([('NEW'),('REVISED')],zero ='Choose Circular Type')),
    Field('supplier_code_id', 'reference Supplier_Master', ondelete = 'NO ACTION',requires = IS_IN_DB(db, db.Supplier_Master.id, '%(supp_code)s - %(supp_name)s', zero =  'Choose Supplier')),      
    Field('dept_code_id','reference Department', ondelete = 'NO ACTION',label = 'Dept Code',requires = IS_IN_DB(db, db.Department.id,'%(dept_code)s - %(dept_name)s', zero = 'Choose Department')),       
    Field('acct_approved_by', 'reference auth_user', ondelete = 'NO ACTION', writable = False),
    Field('acct_approved_date', 'datetime', writable = False),
    Field('mngt_approved_by','reference auth_user', ondelete = 'NO ACTION', writable = False),
    Field('mngt_approved_date','datetime', writable = False),    
    Field('remarks','string'),
    Field('status_id','reference Stock_Status', ondelete = 'NO ACTION',requires = IS_IN_DB(db, db.Stock_Status.id, '%(description)s', zero = 'Choose Status')),       
...
db.define_table('Price_Circular_Transaction',
    Field('circular_no_id','reference Price_Circular',ondelete = 'NO ACTION',writable = False),
    Field('item_code_id', 'reference Item_Master', ondelete = 'NO ACTION',requires = IS_IN_DB(db, db.Item_Master.id, '%(item_code)s', zero = 'Choose Item Code')),        
    Field('retail_price', 'decimal(20,6)',default = 0),        
    Field('discount_percentage', 'decimal(20,2)',default =0),    
    Field('wholesale_price', 'decimal(20,6)', default = 0),
    Field('vansale_price', 'decimal(20,6)',default =0),
    Field('retail_margin','decimal(20,6)',default=0),
    Field('wholesales_margin','decimal(20,6)',default=0),
    Field('landed_cost_margin','decimal(20,6)',default=0),
    Field('delete', 'boolean', default = False),
    Field('created_on', 'datetime', default=request.now, writable = False, readable = False),
    Field('created_by', 'reference auth_user', ondelete = 'NO ACTION',default = auth.user_id, writable = False, readable = False, represent = lambda row: row.first_name.upper() + ' ' + row.last_name.upper()),
    Field('updated_on', 'datetime', update=request.now, writable = False, readable = False),
...
db.define_table('Price_Circular_Transaction_Temporary',
    Field('item_code', 'string', length = 25),    
    Field('item_code_id', 'reference Item_Master', ondelete = 'NO ACTION',requires = IS_IN_DB(db, db.Item_Master.id, '%(item_code)s', zero = 'Choose Item Code')),        
    Field('retail_price', 'decimal(20,6)',default = 0),        
    Field('discount_percentage', 'decimal(20,2)',default =0),    
...
db.define_table('Year_End_Report',
    ...
    Field('department_code', 'integer'),
    Field('department_name', 'string', length = 50),
    Field('year_end','string',length=4),    
    Field('month_end','integer'),
    Field('month_name','string',length=25),    
