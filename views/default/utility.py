...
@auth.requires_login()
def get_stock_adjustment_utility_grid():
    row = []
...
def put_stock_adjustment_consolidation():    
    for n in db((db.Stock_Adjustment.srn_status_id == 15) & (db.Stock_Adjustment.processed == False)).select(orderby = db.Stock_Adjustment.id):
        _chk = db((db.Merch_Stock_Header.voucher_no == int(n.stock_adjustment_no)) & (db.Merch_Stock_Header.transaction_type == 7) & (db.Merch_Stock_Header.location == n.location_code_id)).select().first()
        if not _chk:          
            ...
            db.Merch_Stock_Header.insert(
                ...
                account = n.stock_adjustment_code,
                ...
                batch_code_id = put_batch_file())
            _id = db((db.Merch_Stock_Header.voucher_no == n.stock_adjustment_no) & (db.Merch_Stock_Header.transaction_type == 7)).select().first()
            print '_id:', _id.id
...
@auth.requires_login()
def get_stock_corrections_utility_grid():
    row = []
...
def put_stock_corrections_consolidation():
    _ctr = db(db.Dbf_Batch_Table).count() + 1
    _batch_gen = str(request.now.year)+str(request.now.month)+str(request.now.day) + str(_ctr)    
    db.Dbf_Batch_Table.insert(batch_code = _batch_gen, status_id = 1)
    _batch_id = db().select(db.Dbf_Batch_Table.ALL).last()    
