...
def get_pending_invoice_for_delivery_canvas(canvas, p_doc):
    canvas.saveState()
    _header = [['Pending Invoice for Delivery']]
...
def get_pending_sales_return_report():    
    ...
    _row = [['#','For SRS','Amount','Good Receipt No.','SR.No.','Date','SI.No.','Department','Customer','Total Amount','Req.By']]    
    ctr = 0
    for n in db((db.Sales_Invoice.sales_invoice_date_approved >= session.from_date) & (db.Sales_Invoice.sales_invoice_date_approved <= session.to_date) & (db.Sales_Invoice.status_id == 7) & (db.Sales_Invoice.dept_code_id == _usr.department_id) & (db.Sales_Invoice.returned == True) & (db.Sales_Invoice.returned_processed != True)).select(orderby = ~db.Sales_Invoice.sales_invoice_date_approved | ~db.Sales_Invoice.customer_code_id):
        ctr += 1        
        _row.append([
            ctr,
            n.returned,
    ...
    _table = Table(_row, colWidths=[20,40,50,60,60,60,60,70,'*',70,70], repeatRows=1)
    _table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        ('FONTNAME',(0,0),(-1,-1), 'Courier'),
        ('FONTSIZE',(0,0),(-1,-1),8),
...
def get_pending_invoice_for_delivery_report():    
    _usr = db(db.Warehouse_Manager_User.user_id == auth.user_id).select().first()    
    _row = [['#','Delivered','SR.No.','Date','SI.No.','Department','Customer','Total Amount','Req.By']]    
    ctr = 0    
    for n in db((db.Sales_Invoice.sales_invoice_date_approved >= session.from_date) & (db.Sales_Invoice.sales_invoice_date_approved <= session.to_date) & (db.Sales_Invoice.status_id == 7) & (db.Sales_Invoice.dept_code_id == _usr.department_id) & (db.Sales_Invoice.delivered == False)).select(orderby = db.Sales_Invoice.customer_code_id):
        ctr += 1        
        _row.append([
            ctr,
            n.delivered,
            ...
            locale.format('%.2F',n.total_amount_after_discount or 0, grouping = True),
            n.sales_man_id.employee_id.first_name])
    _table = Table(_row, colWidths=[20,50,60,60,60,70,'*',70,70], repeatRows=1)
    _table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        ('FONTNAME',(0,0),(-1,-1), 'Courier'),
        ('FONTSIZE',(0,0),(-1,-1),8),
    ...
    row.append(_table)
    p_doc.pagesize = landscape(A4)
    p_doc.build(row, onFirstPage= get_pending_invoice_for_delivery_canvas, onLaterPages = get_pending_invoice_for_delivery_canvas, canvasmaker=LandscapePageNumCanvas)
    pdf_data = open(tmpfilename,"rb").read()
    os.unlink(tmpfilename)
    response.headers['Content-Type']='application/pdf'    
    return pdf_data    

def get_direct_purchase_receipt_report_header(canvas, w_doc):
    canvas.saveState()
...
def get_direct_purchase_receipt_id():       
    ...
    for n in db((db.Direct_Purchase_Receipt_Transaction.purchase_receipt_no_id == request.args(0)) & (db.Direct_Purchase_Receipt_Transaction.quantity != 0) & (db.Direct_Purchase_Receipt_Transaction.delete == False)).select(orderby = db.Direct_Purchase_Receipt_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Direct_Purchase_Receipt_Transaction.item_code_id)):
        ctr += 1
        if int(n.Direct_Purchase_Receipt_Transaction.quantity) != 0:
            _quantity_received = card(n.Direct_Purchase_Receipt_Transaction.quantity,n.Direct_Purchase_Receipt_Transaction.uom)
        _row.append([
            ctr,
            n.Direct_Purchase_Receipt_Transaction.item_code_id.item_code,
            str(n.Item_Master.brand_line_code_id.brand_line_name) + str('\n') + str(n.Item_Master.item_description),            
            n.Direct_Purchase_Receipt_Transaction.uom,
            n.Direct_Purchase_Receipt_Transaction.category_id.description,
            _quantity_received])    
    _table = Table(_row, colWidths=[20,70,'*',60,60,70], repeatRows = 1)
    _table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        ('LINEABOVE', (0,0), (-1,0), 0.25, colors.black,None, (2,2)),
        ('LINEBELOW', (0,0), (-1,0), 0.25, colors.black,None, (2,2)),
    ...
    row.append(_w_table)
    row.append(PageBreak())
    w_doc.build(row, onFirstPage= get_direct_purchase_receipt_report_header, onLaterPages = get_direct_purchase_receipt_report_header, canvasmaker=PageNumCanvas)
    pdf_data = open(tmpfilename,"rb").read()
    os.unlink(tmpfilename)
    response.headers['Content-Type']='application/pdf'    
    return pdf_data
