...
def sales_order_form():    
    ...
    if form.process().accepted:        
        ...
        _id.update_record(total_amount = _grand_total, discount_percentage = _discount, total_amount_after_discount = _after_discount, total_selective_tax = _total_selective_tax, total_selective_tax_foc = _total_selective_tax_foc)
        session.discount = 0
...
def get_sales_invoice_workflow_reports():    
    row = []
    _usr = db(db.Sales_Man.users_id == auth.user_id).select().first()
    print 'user: ', _usr.employee_id, _usr.users_id, _usr.id
    head = THEAD(TR(TH('Date'),TH('Sales Order No.'),TH('Delivery Note No.'),TH('Sales Invoice No.'),TH('Department'),TH('Customer'),TH('Location Source'),TH('Amount'),TH('Status'),TH('Action Required'),TH('Action')),_class='bg-primary')
    # for n in db((db.Sales_Order.created_by == auth.user.id) & (db.Sales_Order.archives == False)).select(orderby = ~db.Sales_Order.id):  
    for n in db((db.Sales_Invoice.sales_man_id == _usr.id) & (db.Sales_Invoice.archives == False) & (db.Sales_Invoice.status_id == 7)).select(orderby = ~db.Sales_Invoice.id):          
        # if n.status_id == 7:
        #     clea_lnk = A(I(_class='fas fa-archive'), _title='Clear Row', _type='button ', _role='button', _class='btn btn-icon-toggle clear', callback = URL(args = n.id, extension = False), **{'_data-id':(n.id)})            
        #     view_lnk = A(I(_class='fas fa-search'), _title='View Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('sales','sales_order_view', args = n.id, extension = False))        
        # else:
...
def get_sales_invoice_grid():
    ...
    for n in db(db.Sales_Invoice.status_id == 7).select(orderby = ~db.Sales_Invoice.id):  
        if n.status_id == 7:            
            ...
            view_lnk = A(I(_class='fas fa-search'), _title='View Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('sales','get_sales_invoice_no_id', args = n.id, extension = False))        
...
def get_sales_invoice_no_id():
    db.Sales_Invoice.sales_order_date.writable = False
    db.Sales_Invoice.dept_code_id.writable = False
    db.Sales_Invoice.stock_source_id.writable = False
    db.Sales_Invoice.customer_code_id.writable = False
    db.Sales_Invoice.customer_order_reference.writable = False
    db.Sales_Invoice.delivery_due_date.writable = False
    db.Sales_Invoice.total_amount.writable = False
    db.Sales_Invoice.total_selective_tax.writable = False
    db.Sales_Invoice.total_vat_amount.writable = False    
    db.Sales_Invoice.sales_man_id.writable = False    
    db.Sales_Invoice.section_id.writable = False
    db.Sales_Invoice.status_id.requires = IS_IN_DB(db((db.Stock_Status.id == 7) | (db.Stock_Status.id == 8)), db.Stock_Status.id, '%(description)s', zero = 'Choose Status')
    db.Sales_Invoice.status_id.default = 8
    _id = db(db.Sales_Invoice.id == request.args(0)).select().first()
    form = SQLFORM(db.Sales_Invoice, request.args(0))
    if form.process().accepted:
        session.flash = 'RECORD UPDATED'
...
@auth.requires_login()    
def get_sales_invoice_transaction_id():
    ctr = 0
    row = []                
    _grand_total = 0
    _total_amount = 0        
    ...
    for n in _query:
    # for n in db((db.Sales_Invoice_Transaction.de == session.sales_order_no_id) & (db.Sales_Invoice_Transaction.delete == False)).select(db.Sales_Invoice_Transaction.ALL, db.Item_Master.ALL,orderby = ~db.Sales_Invoice_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Sales_Invoice_Transaction.item_code_id)):
        ctr += 1

        # discount & grand total computation
        _grand_total += float(n.Sales_Invoice_Transaction.total_amount or 0)
        ...
        if _selective_tax_foc > 0.0:
            ...
            _div_tax_foc = DIV('')
        # ownership 
        if auth.user_id != n.Sales_Invoice_Transaction.created_by:
            ...
            if _id.status_id == 4:
                edit_lnk = A(I(_class='fas fa-pencil-alt'), _title='Edit Row', _class='btn btn-icon-toggle', _href=URL('sales','sales_order_edit_view', args = n.Sales_Invoice_Transaction.id, extension = False))
                dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _class='btn btn-icon-toggle delete', callback = URL( args = n.Sales_Invoice_Transaction.id, extension = False), **{'_data-id':(n.Sales_Invoice_Transaction.id)})
            else:
                edit_lnk = A(I(_class='fas fa-pencil-alt'), _title='Edit Row', _class='btn btn-icon-toggle disabled')            
                dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row',_class='btn btn-icon-toggle disabled')

        btn_lnk = DIV( dele_lnk)
        _qty = n.Sales_Invoice_Transaction.quantity / n.Sales_Invoice_Transaction.uom        
        _pcs = n.Sales_Invoice_Transaction.quantity - n.Sales_Invoice_Transaction.quantity / n.Sales_Invoice_Transaction.uom * n.Sales_Invoice_Transaction.uom        
        
        _cst = (n.Sales_Invoice_Transaction.price_cost * n.Sales_Invoice_Transaction.uom) + (n.Sales_Invoice_Transaction.selective_tax / n.Sales_Invoice_Transaction.uom)
        # _pri = _qty * n.Sales_Invoice_Transaction.uom
        if db((db.Sales_Invoice.id == request.args(0)) & (db.Sales_Invoice.status_id == 7) | (db.Sales_Invoice.created_by != auth.user_id)).select().first():
            _btnUpdate = INPUT(_id='btnUpdate', _name='btnUpdate', _type= 'submit', _value='update', _class='btn btn-success', _disabled = True)
        else:
            _btnUpdate = INPUT(_id='btnUpdate', _name='btnUpdate', _type= 'submit', _value='update', _class='btn btn-success')
        row.append(TR(
            TD(ctr),
            TD(n.Sales_Invoice_Transaction.item_code_id.item_code),
            TD(n.Item_Master.brand_line_code_id.brand_line_name),
            TD(n.Item_Master.item_description),
            TD(n.Sales_Invoice_Transaction.category_id.mnemonic, _style = 'width:120px'),
            TD(n.Sales_Invoice_Transaction.uom, _style = 'width:120px'),
            TD(_qty, _style = 'width:80px'),
            TD(_pcs, _style = 'width:80px'),
            TD(locale.format('%.2F',n.Sales_Invoice_Transaction.price_cost or 0, grouping = True), _align = 'right', _style = 'width:100px'),
            TD(locale.format('%d',n.Sales_Invoice_Transaction.discount_percentage), _align = 'right', _style = 'width:80px'),
            TD(locale.format('%.2F',n.Sales_Invoice_Transaction.net_price or 0, grouping = True), _align = 'right', _style = 'width:100px'),
            TD(locale.format('%.2F',n.Sales_Invoice_Transaction.total_amount or 0,grouping = True), _align = 'right', _style = 'width:100px'),
            TD(btn_lnk)))
    body = TBODY(*row)
    foot = TFOOT(TR(TD(),TD(_div_tax_foc),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('TOTAL AMOUNT', _align = 'right',_colspan='2'),TD(locale.format('%.2F',_grand_total or 0, grouping = True), _align = 'right'),TD()))
    foot += TFOOT(TR(TD(),TD(_div_tax),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('DISCOUNT %', _align = 'right',_colspan='2'),TD(locale.format('%d',0 or 0, grouping = True), _align = 'right'),TD()))
...
@auth.requires_login()    
def get_delivery_note_transaction_id():
    ctr = 0
    row = []                
    _grand_total = 0
    _total_amount = 0        
    ...
    _query = db((db.Delivery_Note_Transaction.delivery_note_id == request.args(0)) & (db.Delivery_Note_Transaction.delete == False)).select(db.Delivery_Note_Transaction.ALL, db.Item_Master.ALL,orderby = ~db.Delivery_Note_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Delivery_Note_Transaction.item_code_id))    
    head = THEAD(TR(TH('#'),TH('Item Code'),TH('Brand Line'),TH('Item Description'),TH('Category'),TH('UOM'),TH('Quantity'),TH('Pieces'),TH('Unit Price/Sel.Tax'),TH('Discount %'),TH('Net Price'),TH('Total Amount'),TH('Action'),_class='bg-primary'))
    for n in _query:
    # for n in db((db.Delivery_Note_Transaction.de == session.sales_order_no_id) & (db.Delivery_Note_Transaction.delete == False)).select(db.Delivery_Note_Transaction.ALL, db.Item_Master.ALL,orderby = ~db.Delivery_Note_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Delivery_Note_Transaction.item_code_id)):
        ctr += 1

        # discount & grand total computation
        _grand_total += float(n.Delivery_Note_Transaction.total_amount or 0)
        _discount = float(_grand_total) * int(_id.discount_percentage or 0) / 100        
        _grand_total = float(_grand_total) - int(_discount)                
        ...
        if _selective_tax_foc > 0.0:
            ...
            _div_tax_foc = DIV('')
        # ownership 
        if auth.user_id != n.Delivery_Note_Transaction.created_by:
            edit_lnk = A(I(_class='fas fa-pencil-alt'), _title='Edit Row', _class='btn btn-icon-toggle disabled')            
            dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row',_class='btn btn-icon-toggle disabled')
        else:
            if _id.status_id == 4:
                edit_lnk = A(I(_class='fas fa-pencil-alt'), _title='Edit Row', _class='btn btn-icon-toggle', _href=URL('sales','sales_order_edit_view', args = n.Delivery_Note_Transaction.id, extension = False))
                dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _class='btn btn-icon-toggle delete', callback = URL( args = n.Delivery_Note_Transaction.id, extension = False), **{'_data-id':(n.Delivery_Note_Transaction.id)})
            else:
                edit_lnk = A(I(_class='fas fa-pencil-alt'), _title='Edit Row', _class='btn btn-icon-toggle disabled')            
                dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row',_class='btn btn-icon-toggle disabled')

        btn_lnk = DIV( dele_lnk)
        _qty = n.Delivery_Note_Transaction.quantity / n.Delivery_Note_Transaction.uom        
        _pcs = n.Delivery_Note_Transaction.quantity - n.Delivery_Note_Transaction.quantity / n.Delivery_Note_Transaction.uom * n.Delivery_Note_Transaction.uom        
        
        _cst = (n.Delivery_Note_Transaction.price_cost * n.Delivery_Note_Transaction.uom) + (n.Delivery_Note_Transaction.selective_tax / n.Delivery_Note_Transaction.uom)
        # _pri = _qty * n.Delivery_Note_Transaction.uom
        if db((db.Delivery_Note.id == request.args(0)) & (db.Delivery_Note.status_id == 7) | (db.Delivery_Note.created_by != auth.user_id)).select().first():
            _btnUpdate = INPUT(_id='btnUpdate', _name='btnUpdate', _type= 'submit', _value='update', _class='btn btn-success', _disabled = True)
        else:
            _btnUpdate = INPUT(_id='btnUpdate', _name='btnUpdate', _type= 'submit', _value='update', _class='btn btn-success')
        row.append(TR(
            TD(ctr),
            TD(n.Delivery_Note_Transaction.item_code_id.item_code),
            TD(n.Item_Master.brand_line_code_id.brand_line_name),
            TD(n.Item_Master.item_description),
            TD(n.Delivery_Note_Transaction.category_id.mnemonic, _style = 'width:120px'),
            TD(n.Delivery_Note_Transaction.uom, _style = 'width:120px'),
            TD(_qty, _style = 'width:80px'),
            TD(_pcs, _style = 'width:80px'),
            TD(locale.format('%.2F',n.Delivery_Note_Transaction.price_cost or 0, grouping = True), _align = 'right', _style = 'width:100px'),
            TD(locale.format('%d',n.Delivery_Note_Transaction.discount_percentage), _align = 'right', _style = 'width:80px'),
            TD(locale.format('%.2F',n.Delivery_Note_Transaction.net_price or 0, grouping = True), _align = 'right', _style = 'width:100px'),
            TD(locale.format('%.2F',n.Delivery_Note_Transaction.total_amount or 0,grouping = True), _align = 'right', _style = 'width:100px'),
            TD(btn_lnk)))
    body = TBODY(*row)
    foot = TFOOT(TR(TD(),TD(_div_tax_foc),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('TOTAL AMOUNT', _align = 'right',_colspan='2'),TD(locale.format('%.2F',_grand_total or 0, grouping = True), _align = 'right'),TD()))
    foot += TFOOT(TR(TD(),TD(_div_tax),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('DISCOUNT %', _align = 'right',_colspan='2'),TD(locale.format('%d',0 or 0, grouping = True), _align = 'right'),TD()))
...
def get_sales_return_grid():
    row = []
    head = THEAD(TR(TH('Date'),TH('Sales Return No.'),TH('Department'),TH('Customer'),TH('Location'),TH('Amount'),TH('Status'),TH('Action Required'),TH('Action'),_class='bg-primary'))
    for n in db((db.Sales_Return.status_id == 4) & (db.Sales_Return.created_by == auth.user_id)).select(orderby = ~db.Sales_Return.id):  
...
def sales_return_browse():
    ...
    for n in db((db.Sales_Return.created_by == auth.user.id) & (db.Sales_Return.archives == False)).select(orderby = ~db.Sales_Return.id):  
        if n.status_id == 13:            
            clea_lnk = A(I(_class='fas fa-archive'), _title='Clear Row', _type='button ', _role='button', _class='btn btn-icon-toggle clear', callback = URL(args = n.id, extension = False), **{'_data-id':(n.id)})            
            view_lnk = A(I(_class='fas fa-search'), _title='View Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('sales','sales_return_view', args = n.id, extension = False))        
        else:
...
def sales_return_grid():
    ...
    for n in _query:
        ...
        if auth.has_membership(role = 'INVENTORY SALES MANAGER'):
            ...
            reje_lnk = A(I(_class='fas fa-times'), _title='Reject Row', _type='button ', _role='button', _class='btn btn-icon-toggle btn', callback = URL('sales','sales_return_sales_manager_rejected', args = n.id, extension = False))
            prin_lnk = A(I(_class='fas fa-print'), _type='button ', _role='button', _class='btn btn-icon-toggle disabled')
        ...
        if auth.has_membership(role = 'INVENTORY STORE KEEPER'):
            if n.status_id == 14:
                ...
                reje_lnk = A(I(_class='fas fa-times'), _title='Reject Sales Return Request', _type='button ', _role='button', _class='btn btn-icon-toggle btn', callback = URL('sales','sales_return_warehouse_form_reject', args = n.id, extension = False))                
                prin_lnk = A(I(_class='fas fa-print'), _type='button ', _role='button', _class='btn btn-icon-toggle disabled')
...
def get_workflow_reports():
    if int(request.args(0)) == int(1):
        ...
        # else:            
        _query = db((db.Delivery_Note.dept_code_id == 3) & (db.Delivery_Note.status_id == 7)).select(orderby = ~db.Delivery_Note.id)
        ...
        if not _usr:
            ...
            _query = db((db.Sales_Return.archives == False) & (db.Sales_Return.dept_code_id == 3)).select(orderby = ~db.Sales_Return.id)   
        if auth.has_membership(role = 'ACCOUNTS') or auth.has_membership(role = 'ACCOUNT MANAGER'):
    ...
    for n in _query:       
        ...
        if auth.has_membership(role = 'INVENTORY STORE KEEPER'):
            if (n.status_id == 9) or (n.status_id == 1): 
                ...
                # clea_lnk = A(I(_class='fas fa-archive'), _type='button ', _role='button', _class='btn btn-icon-toggle disabled')          
        if auth.has_membership(role = 'ACCOUNT MANAGER') | auth.has_membership(role = 'ACCOUNTS'):
            if n.status_id == 8:
                ...
                reje_lnk = A(I(_class='fas fa-user-times'), _title='Reject Delivery Note', _type='button ', _role='button', _class='btn btn-icon-toggle disabled')#, callback = URL('sales','sale_order_manager_invoice_no_rejected', args = n.id, extension = False))        
            elif n.status_id == 7:            
        ...
        if not n.sales_invoice_no_prefix_id:
            ...
            _inv = A(_inv, _class='text-danger')#, _title='Sales Invoice', _type='button  ', _role='button', **{'_data-toggle':'popover','_data-placement':'right','_data-html':'true','_data-content': invoice_info(n.id)})
        if auth.has_membership(role = 'INVENTORY SALES MANAGER'):            
            row.append(TR(TD(ctr),TD(n.sales_order_date),TD(_sales),TD(n.dept_code_id.dept_name),TD(n.customer_code_id.account_code,' - ',n.customer_code_id.account_name),TD(n.stock_source_id.location_name),TD(locale.format('%.2F',n.total_amount or 0, grouping = True), _align = 'right'),TD(n.created_by.first_name.upper(), ' ',n.created_by.last_name.upper()),TD(n.status_id.description),TD(n.status_id.required_action),TD(btn_lnk)))
        elif auth.has_membership(role = 'INVENTORY STORE KEEPER'):
            row.append(TR(TD(ctr),TD(n.sales_order_date),TD(_sales),TD(_note),TD(n.dept_code_id.dept_name),TD(n.customer_code_id.account_code,' - ',n.customer_code_id.account_name),TD(n.stock_source_id.location_name),TD(locale.format('%.2F',n.total_amount or 0, grouping = True), _align = 'right'),TD(n.created_by.first_name.upper(), ' ',n.created_by.last_name.upper()),TD(n.status_id.description),TD(n.status_id.required_action),TD(btn_lnk)))        
        else:            
...
def sales_info(e = request.args(0)):    
    for n in db(db.Delivery_Note.id == e).select():
...
def invoice_info(e = request.args(0)):
    for n in db(db.Sales_Invoice.id == e).select():
...
def sales_order_view_account_user():
    db.Delivery_Note.sales_order_date.writable = False
    db.Delivery_Note.dept_code_id.writable = False
    db.Delivery_Note.stock_source_id.writable = False
    db.Delivery_Note.customer_code_id.writable = False
    db.Delivery_Note.customer_order_reference.writable = False
    db.Delivery_Note.delivery_due_date.writable = False
    db.Delivery_Note.total_amount.writable = False
    db.Delivery_Note.total_selective_tax.writable = False
    db.Delivery_Note.total_vat_amount.writable = False    
    db.Delivery_Note.sales_man_id.writable = False    
    db.Delivery_Note.section_id.writable = False
    db.Delivery_Note.status_id.requires = IS_IN_DB(db((db.Stock_Status.id == 7) | (db.Stock_Status.id == 8)), db.Stock_Status.id, '%(description)s', zero = 'Choose Status')
    db.Delivery_Note.status_id.default = 8
    _id = db(db.Delivery_Note.id == request.args(0)).select().first()
    form = SQLFORM(db.Delivery_Note, request.args(0))
    if form.process().accepted:
        session.flash = 'RECORD UPDATED'
        # redirect(URL('inventory', 'str_kpr_grid'))
    elif form.errors:
...
def sync_to_delivery_note_db():    
    ...
    _dn = db(db.Delivery_Note.sales_order_no == _id.sales_order_no).select().first()
    for n in db((db.Sales_Order_Transaction.sales_order_no_id == _id.id) & (db.Sales_Order_Transaction.delete == False)).select():
...
def sales_order_manager_invoice_no_approved():
    ...
    if _id.status_id == 7:        
        ...
        db(db.Sales_Order.sales_order_no == _id.sales_order_no).update(status_id = 7, sales_invoice_no_prefix_id = _trns_pfx.id, sales_invoice_no = _skey, sales_invoice_approved_by = auth.user_id, sales_invoice_date_approved = request.now,)
        for n in db(db.Delivery_Note_Transaction.delivery_note_id == request.args(0)).select():
...
def sync_to_sales_invoice_db():
    ...
    db(db.Delivery_Note.sales_order_no == _si.sales_order_no).update(sales_invoice_no_prefix_id = _si.sales_invoice_no_prefix_id,sales_invoice_no=_si.sales_invoice_no,sales_invoice_approved_by=_si.sales_invoice_approved_by,sales_invoice_date_approved=_si.sales_invoice_date_approved,status_id=_si.status_id)
    for n in db((db.Delivery_Note_Transaction.delivery_note_id == _id.id) & (db.Delivery_Note_Transaction.delete == False)).select():
