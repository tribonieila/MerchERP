...
def purchase_receipt_rejected():       
    db(db.Purchase_Request.id == int(request.args(0))).update(status_id = 18, posted = False, submitted = False, draft = False)
    response.js = "$('#tblPR').get(0).reload()"

def purchase_receipt_approved(): # jyothi's approved
    _id = db((db.Purchase_Request.id == request.args(0)) & (db.Purchase_Request.partial == True)).select().first()
    if _id:
        print 'true and update'
    else:
        print 'false and insert'
...
def get_direct_purchase_transaction_id():    
    ...
    body = TBODY(*row)        
    foot = TFOOT(
        TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control grand_total',_type='text', _name = 'grand_total', _id='grand_total', _readonly = True , _value = locale.format('%.3F',grand_total or 0, grouping = True))),TD()),
        TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Added Discount Amount', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control added_discount_amount',_type='number', _name = 'added_discount_amount', _id='added_discount_amount', _value = locale.format('%.3F',_added_discount_amount)), _align = 'right'),TD(P(_id='error'))),
        TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control net_amount', _type='text', _name = 'net_amount', _id='net_amount', _readonly = True,  _value = locale.format('%.3F',net_amount or 0, grouping = True))),TD(INPUT(_class='form-control',_type='text',_name='Landed_Cost',_id='Landed_Cost',_value='0',_hidden='true'))),
        TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount (QR)', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control local_net_amount',_type='text', _name = 'local_net_amount', _id='local_net_amount', _readonly = True, _value = locale.format('%.3F',local_net_amount or 0, grouping = True))),TD()),
        TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Purchase Value (QR):', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control purchase_value',_type='text', _name = 'purchase_value', _id='purchase_value', _disabled = True,_value = locale.format('%.3F',purchase_value or 0, grouping = True))),TD(_btnUpdate, _style="width:100px;")))
...
def purchase_receipt_account_validated_transaction(): # .load
    ...
    for n in db((db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.delete == False) & (db.Purchase_Request_Transaction.delete_receipt == False) & (db.Purchase_Request_Transaction.delete_invoiced == False)).select(orderby = db.Purchase_Request_Transaction.id):                        
        ctr+=1                
        _im = db(db.Item_Master.id == n.item_code_id).select().first()
        if _im:
...
def direct_purchase_receipt_transaction_form():    # form entry
    ...
    for n in _query:
        ...
        row.append(TR(
            ...
            TD(btn_lnk)))
    body = TBODY(*row)        
    foot = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Purchase Value (QR):', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control purchase_value',_type='text', _name = 'purchase_value', _id='purchase_value', _disabled = True,_value = locale.format('%.3F',purchase_value or 0, grouping = True))),TD(_btnUpdate, _style="width:100px;")))    
...
def purchase_request_transaction_view():
    ...
    _id = db(db.Purchase_Request.id == request.args(0)).select().first()
    if _id.supplier_code_address == None:
        _supplier_code_address = 'None'
    else:
        _supplier_code_address = _id.supplier_code_address.contact_person
    ...
    if form.process().accepted:
        ...
        response.flash = 'FORM HAS ERRORS'                
    return dict(form = form, _id = _id, _ex = _ex, row = _id, section_id = _section_id, title=_title, _supplier_code_address = _supplier_code_address)
...
def purchase_request_grid():
    ...
    if auth.has_membership(role = 'INVENTORY SALES MANAGER') & auth.has_membership(role = 'INVENTORY BACK OFFICE FOOD'): # wael approval        
        ...
        _query = db(db.Purchase_Request.status_id != 22).select(orderby = ~db.Purchase_Request.id) 
        head = THEAD(TR(TH('Date'),TH('Purchase Request No.'),TH('Department'),TH('Supplier Code'),TH('Location'),TH('Amount'),TH('Requested by'),TH('Approved by'),TH('Status'),TH('Action Required'),TH('Action'),_class='bg-primary'))
    for n in _query:
...
def get_workflow_reports():
    ...
    if int(request.args(0)) == 1:
        ...
        if form.accepts(request):
            ...
            _query = db((db.Purchase_Request.purchase_receipt_approved_by == auth.user_id) & (db.Purchase_Request.purchase_receipt_date >= request.vars.from_date) & (db.Purchase_Request.purchase_receipt_date <= request.vars.to_date)).select(orderby = ~db.Purchase_Request.id)    
        else:
            title = 'Warehouse Receipt Workflow Reports' # Warehouse receipt reports
            _query = db((db.Purchase_Request.purchase_receipt_approved_by == auth.user_id) & (db.Purchase_Request.purchase_receipt_date == request.vars.from_date)).select(orderby = ~db.Purchase_Request.id)    
        row = []
...
def get_document_register_report():    
    ...
    form = SQLFORM.factory(
        Field('category_id','integer',requires = IS_IN_SET([('1','CIL List'),('2','Supplier')], zero = 'Choose category')),
...
def get_document_register_grid():
    ...
    for n in _query:
        ...
        if n.d1_release == False: # if manoj status
...
def put_document_register_release_id():
    _id = db(db.Document_Register_Purchase_Order.document_register_no_id == request.args(0)).select().first()    
    db(db.Purchase_Request.purchase_order_no == _id.purchase_order_no).update(status_id = 28)
    db(db.Purchase_Order.purchase_order_no == _id.purchase_order_no).update(status_id = 28)
...
def post_document_register_id():
    ...
    _pr.update_record(d1_yes_no = True, draft = True)    
...
def put_document_register_id():
    ...
    if form.process().accepted:
        response.flash = 'Form updated.'        
        # redirect(URL('procurement','get_document_register_grid'))
    elif form.errors:        
        response.flash = 'Form has error.'                
...
def get_other_payment_schedule_grid():        
    ...
    for n in db().select(orderby = db.Other_Payment_Schedule.id):        
        ...
        dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _type='button  ', _role='button', _class='btn btn-icon-toggle disabled', _href=URL('#', args = n.id))
        btn_lnk = DIV(view_lnk, edit_lnk, dele_lnk)
        row.append(TR(TD(n.serial_no),TD(n.payment_date),TD(n.order_no),TD(n.supplier_name),TD(n.trade_terms),TD(locale.format('%.2F',n.foreign_currency_amount or 0, grouping = True)),TD(locale.format('%.2F',n.local_currency_amount or 0, grouping = True)),TD(n.paid),TD(btn_lnk)))
    body = TBODY(*row)
    table = TABLE(*[head, body], _class='table')
    return dict(table = table)
...
def purchase_receipt_warehouse_grid_consolidated_view():
    ...
    for t in db(db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0) & (db.Purchase_Request_Transaction.delete == False) & (db.Purchase_Request_Transaction.quantity_received_by == auth.user_id)).select(db.Item_Master.ALL, db.Purchase_Request_Transaction.ALL, orderby = db.Purchase_Request_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Request_Transaction.item_code_id)):
        ctr += 1
...
def put_purchase_receipt_transaction():
    ...
    for t in db((db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.delete == False) & (db.Purchase_Request_Transaction.delete_receipt == False)).select(db.Item_Master.ALL, db.Purchase_Request_Transaction.ALL, orderby = db.Purchase_Request_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Request_Transaction.item_code_id)):
        ...
        edit_lnk = A(I(_class='fas fa-pencil-alt'), _title='Edit Row', _type='button  ', _role='button', _class='btn btn-icon-toggle disabled')
        if _id.partial == True:
            revi_lnk = A(I(_class='fas fa-history'), _title='Revive Row', _type='button  ', _role='button', _class='btn btn-success btn-icon-toggle revive', callback=URL('procurement','put_purchase_request_transaction_partial_id',args = t.Purchase_Request_Transaction.id), extension = False, **{'_data-id':(t.Purchase_Request_Transaction.id)})
            if t.Purchase_Request_Transaction.partial == True:
                revi_lnk = A(I(_class='fas fa-redo'), _title='Redo Row', _type='button  ', _role='button', _class='btn btn-success btn-icon-toggle redo', callback=URL('procurement','put_purchase_request_transaction_redo_id',args = t.Purchase_Request_Transaction.id), extension = False, **{'_data-id':(t.Purchase_Request_Transaction.id)})
        else:
        ...
        dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _type='button  ', _role='button', _class='btn btn-danger btn-icon-toggle delete', _id='delete', callback=URL(args = t.Purchase_Request_Transaction.id, extension =False), **{'_data-id':(t.Purchase_Request_Transaction.id)})
        
        btn_lnk = DIV(view_lnk, revi_lnk, dele_lnk)  
        ...
        if t.Purchase_Request_Transaction.partial == True:
            _item_code = SPAN(t.Purchase_Request_Transaction.item_code, _class='badge bg-success')
        else:
            _item_code = t.Purchase_Request_Transaction.item_code
        trow.append(TR(
    ...
    if form.accepts(request, session):        
        ...
        if isinstance(request.vars._id, list):
            ...
            db(db.Purchase_Request_Transaction.id == request.vars._id).update(quantity_received = _qty, production_date = request.vars.production_date, expiration_date=request.vars.expiration_date,quantity_received_by=auth.user_id)
    elif form.errors:
        response.flash = 'FORM HAS ERROR'
    return dict(form = form)    
...
def generate_direct_supplier_code_id():            
    if request.vars.supplier_account_code == 'Supplier Account':
        ...
        response.js = "$('#account_code').prop('readonly', true);$('#account_code').val('%s');" % (_val)
        ...
        response.js = "$('#account_code').prop('readonly', false); $('#account_code').val('');"
...
def get_warehouse_receipt_report_header(canvas, w_doc):
    ...
    if _id.purchase_receipt_no_prefix_id == None:
        ...
        _purchase_receipt = str(_id.purchase_receipt_no_prefix_id.prefix)+str(_id.purchase_receipt_no)
        _purchase_receipt_date = _id.purchase_receipt_date_approved    
    _header = [
        ['WAREHOUSE PURCHASE RECEIPT'],        
        ['Purchase Receipt No.',':',_purchase_receipt,'','Purchase Receipt Date',':',_purchase_receipt_date.strftime('%d/%b/%Y')],        
        ['Purchase Order No.',':',_id.purchase_order_no,'','Supplier Name',':',Paragraph(_id.supplier_code_id.supp_name,_stylePR)],
...
def warehouse_receipt_workflow_reports():   
    ...
    _row = [['#','Item Code','Item Description','UOM','Category','Qty']]
    for n in db((db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.delete == False) & (db.Purchase_Request_Transaction.quantity_received_by == auth.user_id)).select(orderby = db.Purchase_Request_Transaction.id, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Request_Transaction.item_code_id)):
        ctr += 1
        _row.append([
            ctr,
            n.Purchase_Request_Transaction.item_code_id.item_code,
    ...
    _table = Table(_row, colWidths=[20,70,'*',30,80,55,65,90], repeatRows = 1)
    _table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        # ('LINEABOVE', (0,0), (-1,0), 0.25, colors.Color(0, 0, 0, 1)),        
        ('LINEABOVE', (0,0), (-1,0), 0.25, colors.black,None, (2,2)),
...
def purchase_receipt_reports(): # from workflow reports
    ...
    _header = [
        ...
        ['Purchase Order No.',':',str(_pr.purchase_order_no_prefix_id.prefix) + str(_pr.purchase_order_no),'', 'Proforma Invoice ',':',_pr.supplier_reference_order],
...
def document_register_report():
    ...
    _header = [
        ...
        ['Terms of Payment: 70 days from the invoice date'],
